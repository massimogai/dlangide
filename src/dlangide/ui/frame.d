...
bool isSupportedSourceTextFileFormat(string filename)
{
    return (filename.endsWith(".d") || filename.endsWith(".di")
            || filename.endsWith(".dt") || filename.endsWith(".txt")
            || filename.endsWith(".cpp") || filename.endsWith(".h") || filename.endsWith(".c")
            || filename.endsWith(".json") || filename.endsWith(".sdl")
            ...
            || filename.endsWith(".log") || filename.endsWith(".hpp"));
}
...
class BackgroundOperationWatcherTest : BackgroundOperationWatcher
{
    ...
    {
        return "Test progress: "d ~ to!dstring(_counter);
    }
    /// returns icon of background operation to show in status line
    override @property string icon()
    {
        return "folder";
    }
...
class IDEFrame : AppFrame, ProgramExecutionStatusListener,
    BreakpointListChangeListener, BookmarkListChangeListener
{
    ...
    @property DockHost dockHost()
    {
        return _dockHost;
    }

    @property OutputPanel logPanel()
    {
        return _logPanel;
    }
    ...
    protected void onProgramExecutionStatus(ProgramExecution process,
            ExecutionStatus status, int exitCode)
    {
        executeInUiThread(delegate() {
            Log.d("onProgramExecutionStatus process: ", process.executableFile,
            ...
            case ExecutionStatus.Finished:
                _logPanel.logLine(
                    "Program " ~ process.executableFile ~ " finished with exit code " ~ to!string(
                    exitCode));
                break;
    ...
    protected void buildAndDebugProject(Project project)
    {
        ...
        if (!project)
        {
            window.showMessageBox(UIString.fromId("ERROR_CANNOT_DEBUG_PROJECT"c),
                    UIString.fromId("ERROR_STARTUP_PROJECT_ABSENT"c));
            return;
        }
    ...
    protected void debugProject(Project project)
    {
        ...
        {
            window.showMessageBox(UIString.fromId("ERROR_CANNOT_DEBUG_PROJECT"c),
                    UIString.fromId("ERROR_CANNOT_FIND_EXEC"c));
            return;
        }
        ...
        {
            window.showMessageBox(UIString.fromId("ERROR_CANNOT_DEBUG_PROJECT"c),
                    UIString.fromId("ERROR_NO_DEBUGGER"c));
            return;
        }
    ...
    protected void buildAndRunProject(Project project)
    {
        ...
        if (!project)
        {
            window.showMessageBox(UIString.fromId("ERROR_CANNOT_RUN_PROJECT"c),
                    UIString.fromId("ERROR_CANNOT_RUN_PROJECT"c));
            return;
        }
        buildProject(BuildOperation.Build, project, delegate(int result) {
            if (!result)
    ...
    bool setExecutableParameters(ProgramExecution program, Project project,
            string executableFileName)
    {
        ...
        const auto status = program.isDebugger ? UIString.fromId("DEBUGGING"c)
            .value : UIString.fromId("RUNNING"c).value;
    ...
    void runWithRdmd(string filename)
    {
        ...
        string[] args;
        {
            ...
            auto buildConfig = currentWorkspace ? currentWorkspace.buildConfiguration
    ...
    {
        Log.d("onCompilerLogIssueClick project=", projectname, " file=",
        ...
        ProjectSourceFile sourceFile = _wsPanel.findSourceFileItem(fname,
                isAbsolute(fname) ? true : false, projectname);
    ...
    protected void askForUnsavedEdits(void delegate() onConfirm)
    {
        ...
        window.showMessageBox(header ~ " " ~ toUTF32(baseName(tabId)),
                UIString.fromId("MSG_FILE_CONTENT_CHANGED"c), [ACTION_SAVE,
                ACTION_SAVE_ALL, ACTION_DISCARD_CHANGES, ACTION_DISCARD_ALL, ACTION_CANCEL],
    ...
    protected void onTabClose(string tabId)
    {
        ...
        if (index >= 0)
        {
            ...
            {
                // tab content is modified - ask for confirmation
                window.showMessageBox(UIString.fromId("HEADER_CLOSE_TAB"c),
                        UIString.fromId("MSG_TAB_CONTENT_CHANGED"c) ~ ": " ~ toUTF32(baseName(tabId)),
    ...
    override protected MainMenu createMainMenu()
    {
        ...
        editItem.add(ACTION_EDITOR_FIND, ACTION_EDITOR_FIND_NEXT, ACTION_EDITOR_FIND_PREV,
                ACTION_EDITOR_REPLACE, ACTION_FIND_TEXT, ACTION_EDITOR_TOGGLE_BOOKMARK);
        editItem.addSeparator();
        ...
        viewItem.add(ACTION_WINDOW_SHOW_HOME_SCREEN,
                ACTION_WINDOW_SHOW_WORKSPACE_EXPLORER, ACTION_WINDOW_SHOW_LOG_WINDOW);
        ...
        navItem.add(ACTION_GO_TO_DEFINITION, ACTION_GET_COMPLETIONS,
                ACTION_GET_DOC_COMMENTS, ACTION_GET_PAREN_COMPLETION,
                ACTION_EDITOR_GOTO_PREVIOUS_BOOKMARK,
                ACTION_EDITOR_GOTO_NEXT_BOOKMARK, ACTION_GO_TO_LINE,
                ACTION_GO_TO_PREV_POSITION, ACTION_GO_TO_NEXT_POSITION);

        MenuItem projectItem = new MenuItem(new Action(21, "MENU_PROJECT"));
        projectItem.add(ACTION_PROJECT_SET_STARTUP, ACTION_PROJECT_REFRESH,
                ACTION_PROJECT_UPDATE_DEPENDENCIES, ACTION_PROJECT_SETTINGS);
        ...
        buildItem.add(ACTION_WORKSPACE_BUILD, ACTION_WORKSPACE_REBUILD, ACTION_WORKSPACE_CLEAN,
                ACTION_PROJECT_BUILD, ACTION_PROJECT_REBUILD,
                ACTION_PROJECT_CLEAN, ACTION_RUN_WITH_RDMD);

        MenuItem debugItem = new MenuItem(new Action(23, "MENU_DEBUG"));
        debugItem.add(ACTION_DEBUG_START,
                ACTION_DEBUG_START_NO_DEBUG, ACTION_DEBUG_CONTINUE, ACTION_DEBUG_STOP,
                ACTION_DEBUG_PAUSE, ACTION_DEBUG_RESTART, ACTION_DEBUG_STEP_INTO,
                ACTION_DEBUG_STEP_OVER,
                ACTION_DEBUG_STEP_OUT, ACTION_DEBUG_TOGGLE_BREAKPOINT,
                ACTION_DEBUG_ENABLE_BREAKPOINT, ACTION_DEBUG_DISABLE_BREAKPOINT);
    ...
    override protected ToolBarHost createToolbars()
    {
        ...
        _projectConfigurationCombo = new ToolBarComboBox("projectConfig",
                [ProjectConfiguration.DEFAULT_NAME.to!dstring]); //Updateable
        _projectConfigurationCombo.action = ACTION_PROJECT_CONFIGURATIONS;
        tb.addControl(_projectConfigurationCombo);

        _cbBuildConfiguration = new ToolBarComboBox("buildConfig", ["Debug"d,
                "Release"d, "Unittest"d]);
        _cbBuildConfiguration.itemClick = delegate(Widget source, int index) {
            if (currentWorkspace && index < 3)
            {
                currentWorkspace.buildConfiguration = [BuildConfiguration.Debug,
                    BuildConfiguration.Release, BuildConfiguration.Unittest][index];
            }
            return true;
        };
        ...
        tb.addButtons(ACTION_DEBUG_STOP, ACTION_DEBUG_CONTINUE, ACTION_DEBUG_PAUSE, ACTION_DEBUG_RESTART,
                ACTION_DEBUG_STEP_INTO, ACTION_DEBUG_STEP_OVER, ACTION_DEBUG_STEP_OUT,);
        return res;
    }
    ...
    override bool handleActionStateRequest(const Action a)
    {
        ...
        case IDEActions.WindowShowWorkspaceExplorer:
            a.state = currentWorkspace !is null
                ? ACTION_STATE_ENABLED : ACTION_STATE_DISABLE;
            return true;
        ...
        case IDEActions.FileNew:
            a.state = (currentWorkspace
                    && currentWorkspace.startupProject) ? ACTION_STATE_ENABLED
        ...
        case IDEActions.RevealProjectInExplorer:
            ...
            if (currentWorkspace
                    && currentWorkspace.startupProject && !_currentBackgroundOperation)
                a.state = ACTION_STATE_ENABLED;
            else
                a.state = ACTION_STATE_DISABLE;
            return true;
        case IDEActions.BuildSetConfiguration:
            ...
            if (currentWorkspace && currentWorkspace.startupProject
                ...
                a.state = currentWorkspace.buildConfiguration == a.longParam
                    ? ACTION_STATE_CHECKED : ACTION_STATE_ENABLED;
            else
                a.state = ACTION_STATE_DISABLE;
            return true;
        case IDEActions.ProjectSetConfiguration:
            ...
            if (currentWorkspace
                    && currentWorkspace.startupProject && !_currentBackgroundOperation)
                a.state
                    = currentWorkspace.startupProject.projectConfiguration.name == a.stringParam
                    ? ACTION_STATE_CHECKED : ACTION_STATE_ENABLED;
            else
                a.state = ACTION_STATE_DISABLE;
            return true;
        case IDEActions.RunWithRdmd:
            ...
            if (currentEditor && !_currentBackgroundOperation
                    && currentEditor.id.endsWith(".d"))
                a.state = ACTION_STATE_ENABLED;
            else
                a.state = ACTION_STATE_DISABLE;
            return true;
        case IDEActions.DebugStop:
            a.state = isExecutionActive
                ? ACTION_STATE_ENABLED : ACTION_STATE_DISABLE;
            return true;
        case IDEActions.DebugStart:
        case IDEActions.DebugStartNoDebug:
            if (!isExecutionActive && currentWorkspace
                    && currentWorkspace.startupProject && !_currentBackgroundOperation)
                a.state = ACTION_STATE_ENABLED;
            else
                a.state = ACTION_STATE_DISABLE;
        ...
        case IDEActions.CloseWorkspace:
            a.state = (currentWorkspace !is null
                    && !_currentBackgroundOperation) ? ACTION_STATE_ENABLED : ACTION_STATE_DISABLE;
            return true;
        ...
        case EditorActions.Replace:
            a.state = (currentEditor !is null
                    && !_currentBackgroundOperation) ? ACTION_STATE_ENABLED : ACTION_STATE_DISABLE;
            return true;
        case IDEActions.ViewToggleWhitespaceMarks:
            a.state = _settings.showWhiteSpaceMarks
                ? ACTION_STATE_CHECKED : ACTION_STATE_ENABLED;
            return true;
        case IDEActions.ViewToggleTabPositionMarks:
            a.state = _settings.showTabPositionMarks
                ? ACTION_STATE_CHECKED : ACTION_STATE_ENABLED;
            return true;
        case IDEActions.ViewToggleToolbar:
            a.state = _settings.showToolbar
                ? ACTION_STATE_CHECKED : ACTION_STATE_ENABLED;
            return true;
        case IDEActions.ViewToggleStatusbar:
            a.state = _settings.showStatusbar
                ? ACTION_STATE_CHECKED : ACTION_STATE_ENABLED;
            return true;
    ...
    {
        DMDProfilerLogParserOperation op = new DMDProfilerLogParserOperation(this,
                filename, _logPanel, delegate(DMDTraceLogParser parser) {
                    if (parser)
                    {
                    ...
                    else
                    {
                        ...
                        window.showMessageBox(UIString.fromId("ERROR"c),
                            UIString.fromId("ERROR_FAILED_TO_PARSE_FILE"c));
                    }
                });
    ...
    FileDialog createFileDialog(UIString caption,
            int fileDialogFlags = DialogFlag.Modal | DialogFlag.Resizable
    ...
    override bool handleAction(const Action a)
    {
        if (a)
        {
            ...
            case IDEActions.BuildSetConfiguration:
                ...
                if (currentWorkspace
                        && a.longParam >= BuildConfiguration.min
                        && a.longParam <= BuildConfiguration.max)
                {
                    if (currentWorkspace.buildConfiguration != a.longParam)
                    {
                        ...
                        Log.d("Changing build configuration to ",
                                currentWorkspace.buildConfiguration);
                        _cbBuildConfiguration.selectedItemIndex
                            = currentWorkspace.buildConfiguration;
                    }
                }
                return true;
            case IDEActions.ProjectSetConfiguration:
                if (currentWorkspace
                        && currentWorkspace.startupProject && a.stringParam)
                {
            ...
            case IDEActions.FileSaveAs:
                ...
                dlg.addFilter(FileFilterEntry(UIString.fromId("SOURCE_FILES"c),
                        "*.d;*.dd;*.ddoc;*.di;*.dt;*.dh;*.json;*.sdl;*.xml;*.ini"));
            ...
            case IDEActions.WindowCloseDocument:
                ...
                return true;
            case IDEActions.WindowCloseAllDocuments:
                askForUnsavedEdits(delegate() { closeAllDocuments(); });
                return true;
            case IDEActions.WindowShowHomeScreen:
                showHomeScreen();
                return true;
            ...
            case IDEActions.FileOpenWorkspace:
                ...
                dlg.addFilter(FileFilterEntry(UIString.fromId("WORKSPACE_AND_PROJECT_FILES"c),
                        "*.dlangidews;dub.json;dub.sdl;package.json"));
            ...
            case IDEActions.GoToDefinition:
                if (currentEditor)
                {
                    ...
                    caretHistory.pushNewPosition();
                    currentEditor.editorTool.goToDefinition(currentEditor(),
                            currentEditor.caretPos);
                }
                return true;
            case IDEActions.GotoLine:
                ...
                if (currentEditor)
                {
                    Log.d("Go to line");
                    // Ask user for line
                    window.showInputBox(UIString.fromId("GO_TO_LINE"c),
                            UIString.fromId("GO_TO_LINE"c), ""d, delegate(dstring s) {
                                try
                                {
                                    auto num = to!uint(s);
                                    ...
                                    if (num < 1 || num > currentEditor.content.length)
                                    {
                                        ...
                                        window.showMessageBox(UIString.fromId("ERROR"c),
                                            UIString.fromId("ERROR_NO_SUCH_LINE"c));
                                        return;
                                    }
                                    // Go to line
                                    caretHistory.pushNewPosition();
                                ...
                                catch (ConvException e)
                                {
                                    currentEditor.setFocus();
                                    window.showMessageBox(UIString.fromId("ERROR"c),
                                        UIString.fromId("ERROR_INVALID_NUMBER"c));
                                }
                            });
                }
                return true;
            ...
            case IDEActions.GetDocComments:
                ...
                currentEditor.editorTool.getDocComments(currentEditor,
                        currentEditor.caretPos, delegate(string[] results) {
                            if (results.length)
            ...
            case IDEActions.GetCompletionSuggestions:
                ...
                currentEditor.editorTool.getCompletions(currentEditor, currentEditor.caretPos,
                        delegate(dstring[] results, string[] icons, CompletionTypes type) {
                            if (currentEditor)
    ...
    void closeWorkspace()
    {
        ...
        askForUnsavedEdits(delegate() { setWorkspace(null); showHomeScreen(); });
    }
    ...
    void removeProjectItem(const Object obj)
    {
        ...
        window.showMessageBox(UIString.fromId("HEADER_REMOVE_FILE"c),
                ...
                [ACTION_YES, ACTION_NO], 1, delegate(const Action result) {
    ...
    void createNewProject(bool newWorkspace)
    {
        ...
        dlg.dialogResult = delegate(Dialog dlg, const Action result) {
            if (result.id == ACTION_FILE_NEW_PROJECT.id || result.id == ACTION_FILE_NEW_WORKSPACE
                    .id)
            {
    ...
    void showPreferences()
    {
        ...
        SettingsDialog dlg = new SettingsDialog(UIString.fromId("HEADER_SETTINGS"c),
                window, s, createSettingsPages());
        dlg.dialogResult = delegate(Dialog dlg, const Action result) {
    ...
    void showProjectSettings(Project project)
    {
        ...
        SettingsDialog dlg = new SettingsDialog(UIString.fromRaw(project.name ~ " - "d ~ UIString.fromId(
                "HEADER_PROJECT_SETTINGS"c)), window, s, createProjectSettingsPages());
        dlg.dialogResult = delegate(Dialog dlg, const Action result) {
            if (result.id == ACTION_APPLY.id)
            {
                //Log.d("settings after edit:\n", s.toJSON(true));
    ...
    private bool loadProject(Project project)
    {
        if (!project.load())
        {
            ...
            window.showMessageBox(UIString.fromId("ERROR_OPEN_PROJECT"c).value,
                    UIString.fromId("ERROR_OPENING_PROJECT"c).value ~ toUTF32(project.filename));
            return false;
        }
    ...
    void openFileOrWorkspace(string filename)
    {
        ...
        if (filename.isWorkspaceFile)
        {
            ...
            {
                window.showMessageBox(UIString.fromId("ERROR_OPEN_WORKSPACE"c)
                        .value, UIString.fromId("ERROR_OPENING_WORKSPACE"c).value);
                return;
            }
        }
        else if (filename.isProjectFile)
        { // Open non-DlangIDE project file or DlangIDE project
            _logPanel.clear();
            ...
            if (currentWorkspace)
            {
                ...
                if (existing)
                {
                    ...
                    window.showMessageBox(UIString.fromId("MSG_OPEN_PROJECT"c),
                            UIString.fromId("MSG_PROJECT_ALREADY_OPENED"c));
                    return;
                }
                window.showMessageBox(UIString.fromId("MSG_OPEN_PROJECT"c),
                        UIString.fromId("QUESTION_NEW_WORKSPACE"c), [ACTION_ADD_TO_CURRENT_WORKSPACE,
                        ACTION_CREATE_NEW_WORKSPACE, ACTION_CANCEL], 0,
        ...
        {
            _logPanel.logLine("File is not recognized as DlangIDE project or workspace file");
            window.showMessageBox(UIString.fromId("ERROR_INVALID_WORKSPACE_FILE"c),
                    UIString.fromId("ERROR_INVALID_WS_OR_PROJECT_FILE"c));
        }
    }
    ...
    void buildProject(BuildOperation buildOp, Project project, BuildResultListener listener = null)
    {
        ...
        if (!listener)
        {
            if (buildOp == BuildOperation.Upgrade
                    || buildOp == BuildOperation.Build || buildOp == BuildOperation.Rebuild)
            {
        ...
        if (exeFile && (buildOp == BuildOperation.Build || buildOp == BuildOperation.Rebuild
                || buildOp == BuildOperation.Clean || buildOp == BuildOperation.Run))
        {
            ...
            if (exeFile.exists && exeFile.isFile)
            {
                if (!canWrite(exeFile))
                {
                    _logPanel.clear();
                    _logPanel.logLine(
                            "Executable file is in use. Stop runing application before build.");
        ...
        Builder op = new Builder(this, project, _logPanel,
                project.projectConfiguration, currentWorkspace.buildConfiguration,
                buildOp, dubExecutable, dubAdditionalParams, toolchain, arch, listener);
        setBackgroundOperation(op);
    }
    ...
    void updateProjectConfigurations()
    {
        if (currentWorkspace && currentWorkspace.startupProject)
        {
            if (_projectConfigurationCombo)
            {
                ...
                _projectConfigurationCombo.selectedItemIndex
                    = currentWorkspace.startupProject.projectConfigurationIndex;
                _projectConfigurationCombo.itemClick = delegate(Widget source, int index) {
                    if (currentWorkspace)
                    {
                        currentWorkspace.setStartupProjectConfiguration(
                                _projectConfigurationCombo.selectedItem.to!string);
                    }
                    return true;
                };
            }
    ...
    class CaretHistory
    {
        ...
        void pushNewPosition()
        {
            ...
            {
                pushNewPosition(currentEditor().filename,
                        currentEditor.caretPos.line, currentEditor.caretPos.pos);
            }
        }
        ...
        void moveToNext()
        {
            if (currentPos + 1 < caretHistory.length)
            {
                ...
                currentEditor.setCaretPos(caretHistory[currentPos].line,
                        caretHistory[currentPos].pos);
                currentEditor.setFocus();
            }
        }
