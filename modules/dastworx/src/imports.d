...
string[] listFilesImports(string[] files,int a)
{
    mixin(logCall);
    RollbackAllocator allocator;
    StringCache cache = StringCache(StringCache.defaultBucketCount);
    ...
    foreach(fname; files)
    {
        ...
        if (mod.moduleDeclaration)
        {
         writeln('"', mod.moduleDeclaration.moduleName.identifiers.map!(a => a.text).join("."), '"');
        }
...
private final class ImportLister: ASTVisitor
{
    ...
    override void visit(const(ImportDeclaration) decl)
    {
        ...
        foreach (const(SingleImport) si; decl.singleImports)
        {
            ...
            string importName=si.identifierChain.identifiers.map!(a => a.text).join(".");
             if (!dati.canFind(importName)){
             
            dati=dati~importName;
            i++;
             }
        ...
        if (decl.importBindings) with (decl.importBindings.singleImport){
            ...
            string importName=identifierChain.identifiers.map!(a => a.text).join(".");
            if (!dati.canFind(importName)){
